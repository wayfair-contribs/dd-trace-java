...
final class FreemarkerAdviceGeneratorTest extends BaseCsiPluginTest {
  ...
  def 'test invoke dynamic advice'() {
    setup:
    final spec = buildClassSpecification(InvokeDynamicAdvice)
    final generator = buildFreemarkerAdviceGenerator(buildDir)
    ...
    final javaFile = new JavaParser().parse(advice.file).getResult().get()
    final packageDcl = javaFile.getPackageDeclaration().get()
    packageDcl.name.asString() == InvokeDynamicAdvice.package.name
    final adviceClass = javaFile.getType(0)
    adviceClass.name.asString().endsWith(InvokeDynamicAdvice.simpleName + 'After')
    final interfaces = adviceClass.asClassOrInterfaceDeclaration().implementedTypes.collect {it.name.asString() }
    ...
    getStatements(methods['helperClassNames']) == ['return new String[] { "' + InvokeDynamicAdvice.name + '" };']
    getStatements(methods['flags']) == ['return COMPUTE_MAX_STACK;']
    getStatements(methods['apply']) == [
      ...
      'handler.method(Opcodes.INVOKESTATIC, "datadog/trace/plugin/csi/impl/FreemarkerAdviceGeneratorTest$InvokeDynamicAdvice", "after", "([Ljava/lang/Object;Ljava/lang/String;)Ljava/lang/String;", false);'
    ]
  }
