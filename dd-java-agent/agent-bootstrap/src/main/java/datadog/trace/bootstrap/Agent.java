...
public class Agent {
  ...
  private static volatile AgentTaskScheduler.Task<Object> PROFILER_INIT_AFTER_JMX = null;
  ...
  private static synchronized void createAgentClassloader(URL bootstrapURL) {
    if (AGENT_CLASSLOADER == null) {
      try {
        ...
        JarFile jarFile = new JarFile(new File(bootstrapURL.toURI()), false);
        AGENT_CLASSLOADER =
  ...
  private static synchronized void startDatadogAgent(Instrumentation inst) {
    if (null != inst) {
      try {
  ...
  private static void scheduleJmxStart(int jmxStartDelay) {
    if (jmxStartDelay > 0) {
      AgentTaskScheduler.INSTANCE.scheduleWithJitter(
          new JmxStartTask(), UNUSED, jmxStartDelay, TimeUnit.SECONDS);
    } else {
      startJmx();
    }
  }

  static final class JmxStartTask implements AgentTaskScheduler.Task<Object> {
    @Override
    public void run(Object target) {
      startJmx();
    }
    ...
    if (profilingEnabled) {
      ...
      if (PROFILER_INIT_AFTER_JMX != null) {
        if (getJmxStartDelay() == 0) {
          ...
          AgentTaskScheduler.INSTANCE.scheduleWithJitter(
              PROFILER_INIT_AFTER_JMX, UNUSED, 500, TimeUnit.MILLISECONDS);
        } else {
          log.debug("Initializing profiler");
          PROFILER_INIT_AFTER_JMX.run(null);
        }
  ...
  private static synchronized void initializeJmxSystemAccessProvider() {
    if (log.isDebugEnabled()) {
      log.debug("Initializing JMX system access provider for datadog");
    }
    try {
      final Class<?> tracerInstallerClass =
  ...
  private static void startTelemetry(Instrumentation inst, Class<?> scoClass, Object sco) {
    try {
      ...
      log.warn("Unable start telemetry: {}", ex.getMessage());
    }
  }
  ...
  private static void startProfilingAgent(boolean isStartingFirst) {
    final ClassLoader contextLoader = Thread.currentThread().getContextClassLoader();
    try {
      Thread.currentThread().setContextClassLoader(AGENT_CLASSLOADER);
