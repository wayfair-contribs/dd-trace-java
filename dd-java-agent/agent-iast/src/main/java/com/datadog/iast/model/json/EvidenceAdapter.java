  ...
  private void toJsonTaintedValue(
    ...
    for (Range range : ranges) {
      if (range.getStart() > start) {
        writer.value(value.substring(start, range.getStart()));
      }
      writer.beginObject();
      writer.name("value");
      writer.value(value.substring(range.getStart(), range.getStart() + range.getLength()));
      writer.name("source");
    ...
    if (start < value.length()) {
      writer.value(value.substring(start));
    }
    writer.endArray();
  }
