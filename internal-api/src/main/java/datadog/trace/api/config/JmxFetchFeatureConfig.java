...
public class JmxFetchFeatureConfig extends AbstractFeatureConfig {
  ...
  public JmxFetchFeatureConfig(ConfigProvider configProvider, GeneralFeatureConfig generalConfig, TracerFeatureConfig tracerConfig) {
    super(configProvider);
    this.generalConfig = generalConfig;

    boolean runtimeMetricsEnabled = configProvider.getBoolean(RUNTIME_METRICS_ENABLED, true);
    this.jmxFetchEnabled = runtimeMetricsEnabled && configProvider.getBoolean(JMX_FETCH_ENABLED, DEFAULT_JMX_FETCH_ENABLED);
    ...
    this.jmxFetchMetricsConfigs = tryMakeImmutableList(configProvider.getList(JMX_FETCH_METRICS_CONFIGS));
    this.jmxFetchCheckPeriod = configProvider.getInteger(JMX_FETCH_CHECK_PERIOD);
    this.jmxFetchInitialRefreshBeansPeriod = configProvider.getInteger(JMX_FETCH_INITIAL_REFRESH_BEANS_PERIOD);
    ...
    this.jmxFetchStatsdHost = configProvider.getString(JMX_FETCH_STATSD_HOST,
        ...
        null != this.jmxFetchStatsdPort && this.jmxFetchStatsdPort > 0 ? tracerConfig.getAgentHost() : null, DOGSTATSD_HOST);

    this.jmxFetchMultipleRuntimeServicesEnabled = configProvider.getBoolean(JMX_FETCH_MULTIPLE_RUNTIME_SERVICES_ENABLED, DEFAULT_JMX_FETCH_MULTIPLE_RUNTIME_SERVICES_ENABLED);
    this.jmxFetchMultipleRuntimeServicesLimit = configProvider.getInteger(JMX_FETCH_MULTIPLE_RUNTIME_SERVICES_LIMIT, DEFAULT_JMX_FETCH_MULTIPLE_RUNTIME_SERVICES_LIMIT);
  ...
  public Map<String, String> getMergedJmxTags() {
    ...
    final Map<String, String> result = newHashMap(globalTags.size() + this.jmxTags.size() + runtimeTags.size() + 1 /* for serviceName */);
